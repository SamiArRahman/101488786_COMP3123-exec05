...
Answer - express.Router() makes a mini app that we can attach to the main app with app.use(). it helps us by organizing code by feature as in keeping all 'user' routers in routes/users.js and "auth" routes in routes/auth.js

it improves reuseability like we can reuse the same router in multiple apps/services
...
Answer - error handling in express is done with the middleware which has four arguments(err,req,res,next). In routes, call next(err) on failure and add a 404 handler for unmatched paths.
...
Answer - it means that we can use the port from the environment if provided in production otherwise it would default to 8081 which is local. this makes it portable
---
